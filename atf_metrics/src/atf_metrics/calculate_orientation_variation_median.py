...
class CalculateOrientationVariationMedianParamHandler:
    def __init__(self):
        """
    ...
    def parse_parameter(self, testblock_name, params):
        ...
        for metric in params:
            ...
            try:
                ...
                rospy.logwarn("No groundtruth parameters given, skipping groundtruth evaluation for metric 'orientation_variation_median' in testblock '%s'", testblock_name)
                groundtruth = None
                groundtruth_epsilon = None
            metrics.append(CalculateOrientationVariationMedian(metric["root_frame"], metric["measured_frame"], groundtruth, groundtruth_epsilon))
        return metrics

class CalculateOrientationVariationMedian:
    def __init__(self, root_frame, measured_frame, groundtruth, groundtruth_epsilon):
        ...
        self.orientation_variation_median = []
    ...
    def record_tf(self, event):
        if self.active:
            try:
                ...
                orientation_diff = math.sqrt((rot[0] - self.rot_old[0]) ** 2 + (rot[1] - self.rot_old[1]) ** 2 +
                                           (rot[2] - self.rot_old[2]) ** 2)
                self.insert_value(orientation_diff)

                self.trans_old = trans
                self.rot_old = rot

    def insert_value(self, value):
        self.orientation_variation_median.append(value)
        self.orientation_variation_median.sort()
        # if we have 3 items we can remove the largest and the smallest
        if len(self.orientation_variation_median) > 2:
            self.orientation_variation_median.pop()
            self.orientation_variation_median.pop(0)

    def get_median(self):
        if len(self.orientation_variation_median) is 0:
            # this should not happen
            return 0
        if len(self.orientation_variation_median) % 2 == 0:
            # We need to compute the mean of the two middle values; note that there should not be more than 2 values -> c.f. insert_value()
            return (self.orientation_variation_median[0] + self.orientation_variation_median[1]) / 2
        if len(self.orientation_variation_median) % 2 == 1:
            return self.orientation_variation_median.pop((len(self.orientation_variation_median) - 1) / 2)
    ...
    def get_result(self):
        ...
        if self.finished:
            ...
            return "orientation_variation_median", data, groundtruth_result, self.groundtruth, self.groundtruth_epsilon, details
        else:
            return False
