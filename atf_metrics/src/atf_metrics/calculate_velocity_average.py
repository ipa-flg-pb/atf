...
class CalculateVelocityAverageParamHandler:
    def __init__(self):
        """
    ...
    def parse_parameter(self, testblock_name, params):
        ...
        for metric in params:
            ...
            try:
                ...
                rospy.logwarn("No groundtruth parameters given, skipping groundtruth evaluation for metric 'velocity_average' in testblock '%s'", testblock_name)
                groundtruth = None
                groundtruth_epsilon = None
            metrics.append(CalculateVelocityAverage(metric["root_frame"], metric["measured_frame"], groundtruth, groundtruth_epsilon))
        return metrics

class CalculateVelocityAverage:
    ...
    def record_tf(self, event):
        if self.active:
            try:
                ...
                # smoothness summand is euclidean norm of the vector containing the "partial differentials" (for tf_sampling_freq -> infty)
                # might need improvement in regard of numerical stability etc
                smoothness_summand = math.sqrt(((trans[0] - self.trans_old[0]) * self.tf_sampling_freq) ** 2 + ((trans[1] - self.trans_old[1]) * self.tf_sampling_freq) ** 2 +
                ...
                if(self.nr_of_path_segments < 1):
                    ...
                    self.velocity_average = smoothness_summand
                    self.nr_of_path_segments +=1
                else:
                    self.velocity_average = (self.nr_of_path_segments * self.velocity_average + smoothness_summand) / (self.nr_of_path_segments +1)
                    self.nr_of_path_segments +=1
    ...
    def get_result(self):
        ...
        if self.finished:
            ...
            return "velocity_average", data, groundtruth_result, self.groundtruth, self.groundtruth_epsilon, details
        else:
            return False
