...
class CalculateOrientationVariationMaxParamHandler:
    def __init__(self):
        """
    ...
    def parse_parameter(self, testblock_name, params):
        ...
        for metric in params:
            ...
            try:
                ...
                rospy.logwarn("No groundtruth parameters given, skipping groundtruth evaluation for metric 'orientation_variation_max' in testblock '%s'", testblock_name)
                groundtruth = None
                groundtruth_epsilon = None
            metrics.append(CalculateOrientationVariationMax(metric["root_frame"], metric["measured_frame"], groundtruth, groundtruth_epsilon))
        return metrics

class CalculateOrientationVariationMax:
    def __init__(self, root_frame, measured_frame, groundtruth, groundtruth_epsilon):
        ...
        self.orientation_variation_max = 0.0
    ...
    def record_tf(self, event):
        if self.active:
            try:
                ...
                self.orientation_variation_max = max(self.orientation_variation_max, orientation_diff)
    ...
    def get_result(self):
        ...
        if self.finished:
            data = round(self.orientation_variation_max, 3)
            ...
            return "orientation_variation_max", data, groundtruth_result, self.groundtruth, self.groundtruth_epsilon, details
        else:
            return False
