...
class CalculateAccelerationMaxParamHandler:
    def __init__(self):
        """
    ...
    def parse_parameter(self, testblock_name, params):
        ...
        for metric in params:
            ...
            try:
                ...
                rospy.logwarn("No groundtruth parameters given, skipping groundtruth evaluation for metric 'acceleration_max' in testblock '%s'", testblock_name)
                groundtruth = None
                groundtruth_epsilon = None
            metrics.append(CalculateAccelerationMax(metric["root_frame"], metric["measured_frame"], groundtruth, groundtruth_epsilon))
        return metrics

class CalculateAccelerationMax:
    ...
    def record_tf(self, event):
        if self.active:
            try:
                ...
                self.acceleration_max = max(self.acceleration_max, acceleration_value)
    ...
    def get_result(self):
        ...
        if self.finished:
            data = round(self.acceleration_max, 3)
            ...
            return "acceleration_max", data, groundtruth_result, self.groundtruth, self.groundtruth_epsilon, details
        else:
            return False
