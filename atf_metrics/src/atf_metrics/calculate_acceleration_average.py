...
class CalculateAccelerationAverageParamHandler:
    def __init__(self):
        """
    ...
    def parse_parameter(self, testblock_name, params):
        ...
        for metric in params:
            ...
            try:
                ...
                rospy.logwarn("No groundtruth parameters given, skipping groundtruth evaluation for metric 'acceleration_average' in testblock '%s'", testblock_name)
                groundtruth = None
                groundtruth_epsilon = None
            metrics.append(CalculateAccelerationAverage(metric["root_frame"], metric["measured_frame"], groundtruth, groundtruth_epsilon))
        return metrics

class CalculateAccelerationAverage:
    ...
    def record_tf(self, event):
        if self.active:
            try:
                ...
                smoothness_summand = math.sqrt(((trans[0] - 2 * self.trans_old[0] + self.trans_very_old[0]) * (self.tf_sampling_freq ** 2)) ** 2 +
                ...
                if(self.nr_of_path_segments < 1):
                    ...
                    self.acceleration_average = smoothness_summand
                    self.nr_of_path_segments +=1
                else:
                    self.acceleration_average = (self.nr_of_path_segments * self.acceleration_average + smoothness_summand) / (self.nr_of_path_segments +1)
                    self.nr_of_path_segments +=1
    ...
    def get_result(self):
        ...
        if self.finished:
            ...
            return "acceleration_average", data, groundtruth_result, self.groundtruth, self.groundtruth_epsilon, details
        else:
            return False
