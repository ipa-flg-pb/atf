...
class CalculateOrientationVariationAverageParamHandler:
    def __init__(self):
        """
    ...
    def parse_parameter(self, testblock_name, params):
        ...
        for metric in params:
            ...
            try:
                ...
                rospy.logwarn("No groundtruth parameters given, skipping groundtruth evaluation for metric 'orientation_variation_average' in testblock '%s'", testblock_name)
                groundtruth = None
                groundtruth_epsilon = None
            metrics.append(CalculateOrientationVariationAverage(metric["root_frame"], metric["measured_frame"], groundtruth, groundtruth_epsilon))
        return metrics

class CalculateOrientationVariationAverage:
    def __init__(self, root_frame, measured_frame, groundtruth, groundtruth_epsilon):
        """
        Class for calculating the average smoothness of a path followed by the given frame in relation to a given root frame. i.e. the average difference in rotation between two timestamps
        ...
        self.orientation_variation_average = 0.0
        self.tf_sampling_freq = 20.0  # Hz
        self.first_value = True
        self.trans_old = []
        self.rot_old = []
        self.nr_of_path_segments = 0
    ...
    def record_tf(self, event):
        if self.active:
            try:
                ...
                smoothness_summand = math.sqrt((rot[0] - self.rot_old[0]) ** 2 + (rot[1] - self.rot_old[1]) ** 2 +
                                           (rot[2] - self.rot_old[2]) ** 2)
                if(self.nr_of_path_segments < 1):
                    #rospy.logwarn("Transformation: %s, Path Increment: %s",str(trans), str(path_increment))
                    self.orientation_variation_average = smoothness_summand
                    self.nr_of_path_segments +=1
                else:
                    self.orientation_variation_average = (self.nr_of_path_segments * self.orientation_variation_average + smoothness_summand) / (self.nr_of_path_segments +1)
                    self.nr_of_path_segments +=1
    ...
    def get_result(self):
        ...
        if self.finished:
            data = round(self.orientation_variation_average, 3)
            ...
            return "orientation_variation_average", data, groundtruth_result, self.groundtruth, self.groundtruth_epsilon, details
        else:
            return False
